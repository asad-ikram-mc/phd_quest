...
That finding is now a methods paper with my MSc supervisor Dr Ella Haig, "Is the Platform Part of the Measurement?", which treats each platform as a survey mode and specifies a six-step psychometric protocol for machine-labelled text: generalizability theory, measurement invariance and DIF testing, and score linking. Across 500 replications of a known-truth simulation, a pooled index that takes platform scores at face value reports the wrong sign of aggregate sentiment on roughly a quarter of days, purely from measurement differences, while a measurement-adjusted index does not. It goes on arXiv shortly; the project page is #paper.
